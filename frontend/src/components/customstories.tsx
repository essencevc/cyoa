    ...
    <div className="relative h-full w-full max-w-[32rem] transform-gpu rounded-lg border bg-white [box-shadow:0_0_0_1px_rgba(0,0,0,.03),0_2px_4px_rgba(0,0,0,.05),0_12px_24px_rgba(0,0,0,.05)] dark:bg-black dark:[box-shadow:0_-20px_80px_-20px_#ffffff1f_inset] dark:[border:1px_solid_rgba(255,255,255,.1)] md:max-h-[500px]">
      ...
      >
        <div className="flex h-full w-full items-center justify-center rounded-t-xl bg-transparent p-10">
          <motion.div className="flex h-[230px] w-full items-center justify-between gap-x-6">
            <motion.div
              variants={variant1}
              className="z-[3] flex h-fit w-full flex-col items-center justify-between gap-y-2 rounded-md border border-neutral-400/20 bg-white p-6 transition-all duration-100 ease-linear dark:border-neutral-800 dark:bg-neutral-900"
            >
              <div className="text-sm text-neutral-500">Enter the cave</div>
            </motion.div>
            <motion.div
              variants={variant2}
              className="z-[3] flex h-fit w-full flex-col items-center justify-between gap-y-2 rounded-md border border-neutral-400/20 bg-white p-6 transition-all duration-100 ease-linear dark:border-neutral-800 dark:bg-neutral-900"
            >
              <div className="text-sm text-neutral-500">Climb the mountain</div>
            </motion.div>
            <motion.div
              variants={variant3}
              className="z-[3] flex h-fit w-full flex-col items-center justify-between gap-y-2 rounded-md border border-neutral-400/20 bg-white p-6 transition-all duration-100 ease-linear dark:border-neutral-800 dark:bg-neutral-900"
            >
              <div className="text-sm text-neutral-500">Cross the bridge</div>
            </motion.div>
          </motion.div>
        </div>

        <div className="flex w-full flex-col items-start border-t border-neutral-200 p-4 dark:border-neutral-800">
          <h2 className="text-xl font-semibold">Unique Choices</h2>
          <p className="text-base font-normal text-neutral-500 dark:text-neutral-400">
            Every decision shapes your unique story path
          </p>
        </div>
      </motion.div>
    </div>
